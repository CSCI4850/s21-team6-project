...
def decode(notation):
	...
	board = np.zeros((81, 3))
	board[:] = -1
	while index < 81 and strIndex < notation.size():
...
def createMask(board, positions, black):
	mask = np.zeros((81, 3))
	variation = board
	for a in range(81):
		mask[a] = -1 * board[EMPTY, a]
	if positions.size > 0:
		current = ""
		for a in range(81):
			if mask[a]:
				variation = Move(board, a)
				if variation == -1:
					mask[a] = -1
				else:
					position = encode(variation)
...
def main():
	...
	while val != "quit":
		...
		if xC > 8 or xC < 0:
			xC = ord(val[0]) - ord('A')
		yC = ord(val[1]) - ord('0')
		board = Move(board, xC+yC*WIDTH, color)
		color = abs(color-1)
